  ...
  BEFORE INSERT OR UPDATE 
   ON usuarios 
    FOR EACH ROW 
DECLARE
    l_password_changed BOOLEAN := FALSE;
BEGIN 
    IF :new.senha IS NULL THEN
        -- se a nova senha Ã© nula, mantÃ©m a senha antiga
        :new.senha := :old.senha;        
    ELSIF :new.senha <> auth_pkg.custom_hash(:old.login, :old.senha) THEN
        -- verifica se a nova senha Ã© diferente do hash da senha atual
        l_password_changed := TRUE;
    END IF;

    -- se houver mudanÃ§a, salva o hash da nova senha
    IF l_password_changed THEN
        :new.senha := auth_pkg.custom_hash(:new.login, :new.senha);
    END IF;
END;
/
ALTER TRIGGER "TRG_ENCRIPT_SENHA" ENABLE;
  CREATE OR REPLACE EDITIONABLE TRIGGER "USUARIOS_LOGIN_UPCASE_T" 
before
insert or update or delete on "USUARIOS"
for each row
begin
    :new.login := upper(:new.login);
end;
/
ALTER TRIGGER "USUARIOS_LOGIN_UPCASE_T" ENABLE;
...
END;
/
create or replace trigger "USUARIOS_LOGIN_UPCASE_T"
before
insert or update or delete on "USUARIOS"
for each row
begin
...
begin  
  ...
  l_password := sha256.ENCRYPT(l_salt || l_username || p_password);  
...
is  
  ...
  l_boolean         boolean;  
begin  
  -- verifica se o usuÃ¡rio existe e puxa a senha da tabela 
  select senha 
  ...
  -- comparamos se o hash da senha bate com o hash da senha que estÃ¡ na tabela e retorna TRUE ou FALSE 
...
begin  
  select 'Y'  
   ...
   where upper(a.login) = upper(p_username)  
     and a.funcao in ('T','B');  -- verifica se usuario Ã© bibliotecÃ¡rio ou conta de teste/adm
  --  
  return true;  
exception  
when no_data_found then  
...
begin  
  select 'Y'  
    ...
    from usuarios a  
   where upper(a.login) = upper(p_username)  
     and a.funcao in ('E','P');  -- verifica se usuÃ¡rio Ã© estudante ou professor
  --  
  return true;  
exception  
when no_data_found then  
